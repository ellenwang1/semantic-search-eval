...
def load_dataset(split, product_locale):
    ...
    # Query-Product Ranking: Given a user specified query and a list
    # of matched products, the goal of this task is to rank the
...
def preprocess_dataset(df):
    ...
    logging.info(
        f'There are {len(df_deduplicated)} deduplicated rows in this dataset.')

    # Drop all rows where product description is nan
    df_nan = df_deduplicated[df_deduplicated['product_description'].isna()]
    logging.info(
        f'There are {len(df_nan)} nan product descriptions in this dataset.')
    ...
    df_not_nan['product_doc'] = df_not_nan['product_title'] + \
        ' ' + df_not_nan['product_description']
